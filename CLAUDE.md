...
# Knowledge distillation - first generate teacher data, then train (run_kd.py lives under legacy/)
...
CUDA_VISIBLE_DEVICES=0 uv run legacy/run_kd.py --kd-loss-type sft --train-mode svd --train-position output --no-wandb
CUDA_VISIBLE_DEVICES=0 uv run legacy/run_kd.py --kd-loss-type kl --train-mode blocktt --train-position small --no-wandb

# DAPO variant of RL
CUDA_VISIBLE_DEVICES=0 uv run run_rl_dapo.py --train-mode lora --lr 1e-4 --lora-rank 1 --no-wandb
...
python -m unittest tests/test_run_rl_dapo_cli.py       # RL DAPO CLI arg validation
...
- `run_sft.py` — Unified SFT script. `--train-mode full|lora|blocktt|svd` selects the method. Also exports shared utilities (`build_collate_fn`, `build_lr_scheduler`, `build_optimizer`, `prepare_model`, etc.) used by other scripts.
- `run_rl.py` — Unified RL (GRPO) script with same `--train-mode` flag. Uses vLLM for rollout generation.
- `run_rl_dapo.py` — DAPO variant of RL training (alternate policy-gradient recipe); shares most infrastructure with `run_rl.py`.
- `legacy/run_kd.py` — Knowledge distillation script. `--kd-loss-type sft|kl` selects SFT cross-entropy or KL divergence loss. Trains on precomputed teacher data. Imports shared utilities from `run_sft.py`.
