...
DEVICE=2 LR=4e-4 CFG_SUFFIX="--gradient-accumulation-steps=32" NAME_SUFFIX="_bz32" TRAIN_MODE=lora LORA_RANK=64 bash run_sft.sh >/dev/null 2>&1 &
DEVICE=2 LR=6e-5 CFG_SUFFIX="--gradient-accumulation-steps=32" NAME_SUFFIX="_bz32" TRAIN_MODE=full bash run_sft.sh >/dev/null 2>&1 &

DEVICE=2 LR=8e-4 CFG_SUFFIX="--gradient-accumulation-steps=64" NAME_SUFFIX="_bz64" TRAIN_MODE=blocktt DECOMP_MODE=output_one_block TRAIN_POSITION=small S_MERGED_TO=frozen bash run_sft.sh
DEVICE=2 LR=8e-4 CFG_SUFFIX="--gradient-accumulation-steps=64" NAME_SUFFIX="_bz64" TRAIN_MODE=lora LORA_RANK=64 bash run_sft.sh >/dev/null 2>&1 &
DEVICE=2 LR=1e-4 CFG_SUFFIX="--gradient-accumulation-steps=64" NAME_SUFFIX="_bz64" TRAIN_MODE=full bash run_sft.sh >/dev/null 2>&1 &