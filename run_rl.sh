...
run_full()
{
  ...
  local lr="${LR:-1e-5}"
  local run_name="${train_mode}_lr_${lr}"
...
run_lora()
{
  ...
  local run_name="${train_mode}-lr_${lr}-rank_${lora_rank}"
  local device="${DEVICE:-2}"
...
run_svd()
{
  ...
  local device="${DEVICE:-2}"
  local run_name="${train_mode}-lr_${lr}-s_to_${s_merged_to}-train_${train_position}"
...
run_blocktt()
{
  ...
  local train_position="${TRAIN_POSITION:-both}"
  local s_merged_to="${S_MERGED_TO:-split}"
  local device="${DEVICE:-2}"
  local run_name="${train_mode}-lr_${lr}-${decomp_mode}-s_to_${s_merged_to}-train_${train_position}"
