...
def main(args):
    ...
    # Prefer tokenizer files from the declared base model. Many fine-tuned
    # checkpoints (e.g., blocktt/lora dirs) do not include tokenizer assets.
    tokenizer_source = args.base_model if args.base_model else args.model_name_or_path
    print_rank_0(f"Tokenizer source: {tokenizer_source}")
    print_rank_0(f"Model source: {args.model_name_or_path}")
    try:
        tokenizer = load_hf_tokenizer(tokenizer_source, fast_tokenizer=True)
    except Exception as e:
        if tokenizer_source != args.model_name_or_path:
            print_rank_0(
                f"Tokenizer load from '{tokenizer_source}' failed ({e}). "
                f"Falling back to '{args.model_name_or_path}'."
            )
