#!/bin/bash
# SVD-FT math launcher with bs=2, grad_accum=8, no gradient checkpointing.
# Mirrors finetune_math_svd.sh; differs only in batch/accum/ckpt.
...
train_position="${train_position:-output}"
...
OUTPUT=${OUTPUT_SRC_DIR}/math/${MODEL}/svd-bs2gacc8-noctk-lr_${lr}-pos_${train_position}-smerge_${s_merged_to}-type_${trainable_type}-seed_${seed}
